...
export function ClientDetailPageClient({ initialClient, userRole }: ClientDetailPageClientProps) {
  ...
  const quoteColumns: Column<ClientQuoteSummaryDTO>[] = useMemo(() => [
    { type: 'field', key: 'quoteNumber', label: 'N°', render: (q) => q.quoteNumber ?? '—' },
    { type: 'field', key: 'title', label: 'Titre' },
    { type: 'field', key: 'totalAmount', label: 'Montant', render: (q) => `${q.totalAmount.toFixed(2)} €` },
    { type: 'field', key: 'status', label: 'Statut devis', render: (q) => <StatusBadge type="quote" status={q.status as any} /> },
    { type: 'computed', label: 'Facture', render: (q) => <InvoiceStatusCell invoiceStatus={q.invoiceStatus} invoicePaidAt={q.invoicePaidAt} /> },
    { type: 'field', key: 'issuedAt', label: 'Émis le', render: (q) => q.issuedAt ? new Date(q.issuedAt).toLocaleDateString('fr-FR') : '—' },
    { type: 'field', key: 'validUntil', label: 'Valable jusqu\'au', render: (q) => q.validUntil ? new Date(q.validUntil).toLocaleDateString('fr-FR') : '—' },
  ], []);

  const actions = [
    { label: 'Modifier', icon: Pencil, onClick: () => setIsEditing(true), hidden: !canUpdate || isEditing },
    { label: 'Supprimer', icon: Trash2, onClick: () => setIsConfirmDeleteOpen(true), variant: 'danger' as const, hidden: !canDelete || isEditing },
  ];

  return (
    <div className="container mx-auto p-6 max-w-6xl pb-24 sm:pb-6">
      <Link href="/clients" className="mb-2 inline-flex items-center gap-2 text-sm text-gray-600 hover:text-gray-900">
      ...
      {isEditing ? (
        <ClientForm initialData={data} onSubmit={handleUpdate} onCancel={() => setIsEditing(false)} isLoading={mutationLoading} />
      ) : (
        <div className="space-y-6">
          <Card title="Informations">
            <dl className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><dt className="text-sm font-medium text-gray-500">Contact</dt><dd className="mt-1 text-sm text-gray-900">{data.contact || '—'}</dd></div>
              <div><dt className="text-sm font-medium text-gray-500">Email</dt><dd className="mt-1 text-sm text-gray-900">{data.email || '—'}</dd></div>
              <div><dt className="text-sm font-medium text-gray-500">Téléphone</dt><dd className="mt-1 text-sm text-gray-900">{data.phone || '—'}</dd></div>
              <div className="md:col-span-2"><dt className="text-sm font-medium text-gray-500">Adresse</dt><dd className="mt-1 text-sm text-gray-900 whitespace-pre-line">{data.address || '—'}</dd></div>
            </dl>
          </Card>
      ...
      <QuoteSlideOver isOpen={isAddingQuote} onClose={() => setIsAddingQuote(false)} onSuccess={() => mutate()} clientId={data.id} />
