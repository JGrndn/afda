...
export function FamilyDetailPageClient({ 
  initialFamily, 
  userRole 
}: FamilyDetailPageClientProps) {
  ...
  // ✅ Calculs dérivés et colonnes AVANT les returns conditionnels
  ...
  const paymentColumns: Column<PaymentDTO>[] = useMemo(() => [
    ...
    },
    {
      ...
      render: (payment) =>
        payment.cashingDate
          ? new Date(payment.cashingDate).toLocaleDateString('fr-FR')
          : '-',
    },
    {
      type: 'field',
    ...
    ...(canUpdate || canDelete ? [{
      ...
      label: 'Actions',
      render: (payment: PaymentDTO) => (
        <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
          {canUpdate && payment.status === PAYMENT_STATUS.PENDING && payment.paymentType === PAYMENT_TYPE.CHECK && (
  ...
  const handleDeletePaymentClick = (payment: PaymentDTO) => {
