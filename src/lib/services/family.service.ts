...
import { toFamilyDTO, toFamiliesDTO, toFamilyWithMembersDTO } from '@/lib/mappers/family.mapper';
...
export const familyService = {
  async getAll(
  ...
  ): Promise<FamilyDTO[] | FamilyWithMembersDTO[]> {
    const { filters = {}, orderBy, includeMemberCount } = options || {};
    ...
    if (includeMemberCount) {
      ...
      return families.map(toFamilyWithMembersDTO);
    }
  ...
  async getByIdWithMembers(id: number): Promise<FamilyWithMembersDTO | null> {
    const family = await prisma.family.findUnique({
      where: { id },
      include: {
        members: true,
    ...
    return family ? toFamilyWithMembersDTO(family) : null;
  },
